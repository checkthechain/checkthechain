...
class TraceResult(msgspec.Struct, rename='camel'):
    ...
    #typing.Mapping[str, ContractStateDiff]
...
def _initialize_slot(slots_data, key, slot_changes, block_number) -> None:
    if '+' in slot_changes:
        slots_data[key] = {
...
def _integrate_slot_changes(
    slots_data, key, slot_changes, block_number
) -> None:
...
def flatten_slots_data(slots_data):
    return [
...
def combine_slots_data(slots_data_list):
    if len(slots_data_list):
        return {}
    else:
        slots_data = slots_data_list[0].copy()
...
def _combine_slot_data(slots_data, slot_address, other_slot_data):
    if slot_address not in slots_data:
        ...
        if other_slot_data['first_nonzero_block'] is not None:
            if slot_data['first_nonzero_block'] is None:
                slot_data['first_nonzero_block'] = other_slot_data[ 'first_nonzero_block']
            elif (
