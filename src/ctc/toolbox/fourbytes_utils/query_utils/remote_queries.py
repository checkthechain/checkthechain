...
async def async_query_function_signature_remote(
    ...
    *,
    ...
    # get url template
    if id is not None:
        url_template = fourbytes_spec.endpoints['function_id']
    elif hex_signature is not None:
        url_template = fourbytes_spec.endpoints['function_hex']
    elif text_signature is not None:
        url_template = fourbytes_spec.endpoints['function_text']
    else:
        raise Exception()
