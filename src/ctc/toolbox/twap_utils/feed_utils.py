...
async def async_get_contract_type(address, provider=None):
...
async def async_is_chainlink_feed(address, provider=None):
    try:
...
async def async_is_uniswap_v2_pool(address, provider=None):
    try:
...
async def async_is_uniswap_v3_pool(address, provider=None):
    try:
        factory = await rpc.async_eth_call(
            to_address=address,
            function_abi={
...
async def async_is_curve_pool(address, provider=None):
...
async def async_is_sushi_pool(address, provider=None):
    try:
        factory = await rpc.async_eth_call(
            to_address=address,
            function_abi={
...
async def async_is_balancer_v2_pool(address, provider=None):
    try:
        await rpc.async_eth_call(
            to_address=address,
